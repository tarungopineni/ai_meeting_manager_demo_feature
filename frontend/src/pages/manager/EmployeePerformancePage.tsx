          ...
          <div className="h-60 w-full font-mono text-xs">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                ...
                <YAxis stroke="#60607a" unit="%" tick={{ fontSize: 10 }} width={25} />
                <Tooltip
          ...
          <div className="space-y-3">
            {leaderboard.map((e, index) => (
              <div
                ...
                className={`flex items-center gap-3 p-2.5 sm:p-3 rounded-lg border transition-all cursor-pointer ${
                  selectedUser?.id === e.id ? 'bg-accent/10 border-accent/30' : 'bg-surface-raised/40 border-surface-border hover:border-surface-muted'
                }`}
              >
                <div className="w-6 h-6 rounded-full bg-accent/20 flex items-center justify-center text-accent text-xs font-bold font-mono flex-shrink-0">
                  #{index + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-text-primary truncate">{e.name}</p>
                  <p className="text-[10px] text-text-muted truncate">@{e.username}</p>
                </div>
